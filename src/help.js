...
export const HELP = {
  home: {
    ...
    controls: [
      ...
      ['Notifications', 'When another phone confirms a student, this one is told by the operating system. One notification per student, and never about your own taps.'],
  ...
  admin: {
    ...
    controls: [
      ...
      ['Ask for their location', 'Sends a request to that student\u2019s own phone, and only while nobody has accounted for them. They see it, they can refuse, and nothing is sent unless they agree.'],
      ['If they do not answer', 'Silence is never treated as a yes \u2014 no timer turns it on. A student may be hiding from something and staying quiet on purpose. You can still locate them, but only as your own named decision with a written reason, which their phone is told about and which is never recorded as consent.'],
      ['The track', 'Where their phone last reported, on a map, and how long ago. A position is where a phone was, not where a child is \u2014 a person still has to see them.'],
      ['When locating stops', 'By itself: the moment a person confirms them, when the event ends, or after 30 minutes. The student can also switch it off from their own phone at any time.'],
  ...
  student: {
    ...
    controls: [
      ...
      ['If the school asks for your location', 'Only during an emergency, only while nobody has found you, and only if you agree. You can say no, nothing is sent, and nobody is told off.'],
      ['While you are sharing', 'Your phone says so the whole time, and Stop sharing is on the same screen. It also switches off by itself once somebody finds you, when the event ends, or after 30 minutes.'],
      ['If the school turned it on without you', 'It says who did it and the reason they wrote down. You are told because you are allowed to know, and you can still switch it off.'],
